...
public class AcBusinessController extends BaseController {
	...
	@Autowired
	protected RepositoryService repositoryService; //管理流程定义  与流程定义和部署对象相关的Service
	
	@Autowired
	protected RuntimeService runtimeService; 		//与正在执行的流程实例和执行对象相关的Service(执行管理，包括启动、推进、删除流程实例等操作)
	...
	@Autowired
	private UsersService usersService; 		//历史管理(执行完的数据的管理)
...
//		    }
		    if(hai.getAssignee() != null) {
		    	...
		    	pd.put("USERNAME", hai.getAssignee());
		    	try {
