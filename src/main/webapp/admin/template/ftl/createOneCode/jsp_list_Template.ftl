													...
													<c:when test="${r"${not empty varList}"}">
														<c:forEach items="${r"${varList}"}" var="var" varStatus="vs">
															<tr>
																<td>
																  	<div class="checkbox d-inline">
		                                                            	<input type="checkbox" id="zcheckbox${r"${vs.index }"}" name='ids' value="${r"${var."}${objectNameUpper}_ID${r"}"}">
		                                                            	<label  style="max-height: 12px;" for="zcheckbox${r"${vs.index }"}" class="cr"></label>
		                                                        	</div>
																</td>
																<td scope="row">${r"${page.showCount*(page.currentPage-1)+vs.index+1}"}</td>
														<#list fieldList as var>
											...
											<table style="width:100%;margin-top:15px;">
												<tr>
													<td style="vertical-align:top;">
														<shiro:hasPermission name="${objectNameLower}:add"><a class="btn btn-light btn-sm" onclick="add();">新增</a></shiro:hasPermission>
														<shiro:hasPermission name="${objectNameLower}:del"><a class="btn btn-light btn-sm" onclick="makeAll('确定要删除选中的数据吗?');">删除</a></shiro:hasPermission>
													</td>
