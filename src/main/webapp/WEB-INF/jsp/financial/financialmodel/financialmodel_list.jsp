                        ...
                        <div class="page-wrapper">
                            ...
                            <div class="row">
                                ...
                                <div class="col-xl-12">
									...
									<form action="financialmodel/list" method="post" name="Form" id="Form">
									<!-- 检索  -->
										...
										<tr>
											...
											<td style="vertical-align:top;padding-left:5px;">
												...
												<shiro:hasPermission name="fromExcel">
													<a class="btn btn-light btn-sm" onclick="fromExcel();" style="width: 23px;height:30px;margin-top:1px;margin-left: -9px;" title="从EXCEL导入数据源到系统">
													...
													<c:when test="${not empty varList}">
														<c:forEach items="${varList}" var="var" varStatus="vs">
															<tr>
																<td>
																  	<div class="checkbox d-inline">
		                                                            	<input type="checkbox" id="zcheckbox${vs.index }" name='ids' value="${var.FINANCIALMODEL_ID}">
											...
											<table style="width:100%;margin-top:15px;">
												<tr>
													<td style="vertical-align:top;">
														<shiro:hasPermission name="financialmodel:add"><a class="btn btn-light btn-sm" onclick="add();">新增</a></shiro:hasPermission>
														<shiro:hasPermission name="financialmodel:del"><a class="btn btn-light btn-sm" onclick="makeAll('确定要删除选中的数据吗?');">删除</a></shiro:hasPermission>
													</td>
...
<script type="text/javascript">
		...
		function add(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增";
			 diag.URL = '<%=basePath%>financialmodel/goAdd';
			 diag.Width = 600;
			 diag.Height = 550;
		...
		function del(Id){
			swal({
            	...
            	if (willDelete) {
	            	$.ajax({
	        			type: "POST",
	        			url: '<%=basePath%>financialmodel/delete',
	        	    	data: {FINANCIALMODEL_ID:Id,tm:new Date().getTime()},
	        			dataType:'json',
	        			cache: false,
		...
		function edit(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '<%=basePath%>financialmodel/goEdit?FINANCIALMODEL_ID='+Id;
			 diag.Width = 600;
			 diag.Height = 550;
			 diag.Modal = true;				//有无遮罩窗口
			 diag. ShowMaxButton = true;	//最大化按钮
            	...
            	if (willDelete) {
	    			...
	    			if(str==''){
	    				...
	    				if(msg == '确定要删除选中的数据吗?'){
	    					$.ajax({
	    						type: "POST",
	    						url: '<%=basePath%>financialmodel/deleteAll?tm='+new Date().getTime(),
	    				    	data: {DATA_IDS:str},
	    						dataType:'json',
	    						cache: false,
	    						success: function(data){
		...
		//导出excel
		function toExcel(){
			window.location.href='<%=basePath%>financialmodel/excel';
		}
